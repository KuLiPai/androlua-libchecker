...
["android.permission.ACCESS_CHECKIN_PROPERTIES"]="访问登记属性，读取或写入登记checkin数据库属性表的权限 ",
...
["android.permission.ACCESS_SURFACE_FLINGER"]="访问SurfaceFlingerAndroid平台上底层的图形显示支持，一般用于游戏或照相机预览界面和底层模式的屏幕截图",
...
["android.permission.AUTHENTICATE_ACCOUNTS"]="验证账户，允许一个程序通过账户验证方式访问账户管理ACCOUNT_*MANAGER相关信息*",
...
["android.permission.BROADCAST_PACKAGE_REMOVED"]="应用删除时广，当一个应用在删除时触发一个广播 ",
["android.permission.BROADCAST_SMS"]="收到短信时广播，当收到短信时触发一个广播 ",
["android.permission.BROADCAST_STICKY"]="连续广播，允许一个程序收到广播后快速收到下一个广播 ",
["android.permission.BROADCAST_WAP_PUSH"]="WAPPUSH广播，WAPPUSH服务收到后触发一个广播 ",
...
["android.permission.MANAGE_APP_TOKENS"]="程序引用，管理创建、摧毁、Z轴顺序，仅用于系统 ",
...
["android.permission.RECEIVE_WAP_PUSH"]="接收WapPush，接收WAPPUSH信息 ",
...
["android.permission.WRITE_GSERVICES"]="写入Google地图数据，允许程序写入GoogleMap服务数据 ",
